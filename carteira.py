...
def gerar_pdf(cfg: Dict[str, Any]) -> str | None:
    """
    Gera o PDF das caixas usando o DF em cache (não chama rota),
    salva em pdfs_auto/ e APAGA os PDFs antigos dessa pasta.
    """
    try:
        df = load_cache_df()
        meta = load_meta()
        ...
        # APAGA PDFs antigos da pasta local
        for fn in os.listdir(out_dir):
            if fn.lower().endswith(".pdf"):
                try:
                    os.remove(os.path.join(out_dir, fn))
                except Exception as e:
                    print(f"[PDF] Erro ao remover PDF antigo '{fn}': {e}")
        ...
        print(f"[PDF] PDF gerado em: {caminho_pdf}")
        return caminho_pdf
...
def recriar_jobs_automacao():
    ...
    tipo = sch.get("type", "off")
    hours = int(sch.get("hours", 24) or 24)
    weekdays = sch.get("weekdays") or [1, 2, 3, 4, 5]
    specific_times = sch.get("specific_times") or ""

    # remove jobs antigos da automação
    for job in scheduler.get_jobs():
        if job.id.startswith("auto_"):
            scheduler.remove_job(job.id)

    if tipo == "off":
        print("[AUTOMAÇÃO] Desligada (sem jobs).")
        return

    # 1..7 (Seg..Dom) -> 0..6 (APScheduler)
    dias_cron = ",".join(str(d - 1) for d in weekdays if 1 <= d <= 7) or "*"

    if tipo == "every_hours":
        # job simples de intervalo
        ...
        print(f"[AUTOMAÇÃO] A cada {hours}h | próxima: {job.next_run_time}")
        return

    if tipo == "specific_times":
        # pode vir string "13:00;14:00;18:00"
        if isinstance(specific_times, str):
            parts = specific_times.replace(",", ";").split(";")
            specific_times = [p.strip() for p in parts if p.strip()]

        if not specific_times:
            print("[AUTOMAÇÃO] Nenhum horário específico configurado.")
            return

        idx = 0
        for hstr in specific_times:
            try:
                hh, mm = hstr.split(":")
                hh, mm = int(hh), int(mm)
            except ValueError:
                print(f"[AUTOMAÇÃO] Horário inválido ignorado: {hstr}")
                continue

            job_id = f"auto_specific_{idx}"
            idx += 1
            ...
            job = scheduler.add_job(
                ...
                CronTrigger(
                    ...
                    hour=hh,
                    minute=mm,
                    # nada de year=..., month=..., etc.
                ),
                id=job_id,
                misfire_grace_time=300,
            )
            print(f"[AUTOMAÇÃO] Hora específica {hstr} | próxima: {job.next_run_time}")
...
def executar_atualizacao_completa():
    print(f"[AUTOMAÇÃO] Disparou executar_atualizacao_completa às {datetime.now()}")

    # 1) Atualiza dados (mesmo fluxo do botão /run_refresh)
    ok, msg = run_refresh()
    print(f"[AUTOMAÇÃO] Resultado do refresh: {msg}")

    # 2) Gera o PDF usando o cache já atualizado
    cfg = load_config()
    df = load_cache_df()
    meta = load_meta()

    if df is None or meta is None:
        print("[AUTOMAÇÃO] Sem cache/meta após refresh, não gerou PDF.")
        return None

    colmap = meta.get("colmap", {})
    p1, ds1, p2, ds2, p3 = build_dashboard_pivots(df, colmap, cfg.get("fixed_class"))

    pdf_bytes = pivots_to_pdf_bytes(
        p1 if p1 is not None else pd.DataFrame(),
        ds1,
        p2 if p2 is not None else pd.DataFrame(),
        ds2,
        p3 if p3 is not None else pd.DataFrame(),
        "Carteira Logística – Pendências"
    )
    ...
    print(f"[PDF] PDF gerado em: {pdf_path}")

    # 3) Copia pra pasta configurada, SOBREESCREVENDO os antigos
    pdf_folder = cfg.get("schedule", {}).get("pdf_folder") or ""
    if pdf_folder.strip():
        try:
            os.makedirs(pdf_folder, exist_ok=True)

            # apaga PDFs antigos "carteira_*.pdf" na pasta de destino
            for nm in os.listdir(pdf_folder):
                if nm.lower().startswith("carteira_") and nm.lower().endswith(".pdf"):
                    try:
                        os.remove(os.path.join(pdf_folder, nm))
                    except Exception as e:
                        print(f"[AUTOMAÇÃO] Erro ao apagar PDF antigo '{nm}': {e}")

            destino = os.path.join(pdf_folder, os.path.basename(pdf_path))
            shutil.copy2(pdf_path, destino)
            print(f"[AUTOMAÇÃO] PDF copiado para {destino}")
        except Exception as e:
            print(f"[AUTOMAÇÃO] ERRO ao copiar PDF: {e}")
    else:
        print("[AUTOMAÇÃO] Pasta PDF não configurada, não copiou.")
...
def load_excel_df(cfg: Dict[str, Any]) -> Tuple[Optional[pd.DataFrame], Optional[str], List[str]]:
    ...
    if cfg.get("use_direct_file"):
        fp = str(cfg.get("file_path","")).strip()
        if not fp or not os.path.isfile(fp):
            return None, "Arquivo (file_path) não encontrado. Ajuste em Configuração.", warns
        files = [fp]
    else:
        files = _find_files(cfg["data_folder"], cfg["filename_pattern"])
        if not files:
            return None, f"Nenhum arquivo encontrado em '{cfg['data_folder']}' que combine com '{cfg['filename_pattern']}'.", warns
    all_frames=[]
    for path in files:
        try:
            df_one, all_names, used_names, fb = _read_stack_using_official(
                path, cfg.get("official_sheet","Base"), cfg.get("sheet_name","*")
            )
            if fb:
                warns.append(f"Aba oficial não encontrada em '{os.path.basename(path)}'. Usando '{used_names[0]}' como oficial.")
            if df_one is not None and df_one.shape[0]>0:
                df_one["_source_file_"] = os.path.basename(path)
                all_frames.append(df_one)
            else:
                warns.append(f"Nenhuma linha útil em '{os.path.basename(path)}' (verifique se a aba oficial possui dados).")
        except Exception as e:
            return None, f"Erro ao ler Excel '{path}': {e}", warns
    if not all_frames:
        return None, "Nenhum dado após empilhar abas.", warns
    df = pd.concat(all_frames, ignore_index=True)
    src = f"{len(files)} arquivo(s) – mais recente: {os.path.basename(files[0])}"
    return df, src, warns
...
def refresh():
    executar_atualizacao_completa()
    flash("Atualizado com sucesso (dados + PDF).", "success")
    return redirect(url_for("painel"))
...
def atualizar_painel():
    """
    Função utilitária para atualizar via código (não via HTTP).
    Usa exatamente o mesmo fluxo do botão /refresh.
    """
    print(f"[ATUALIZAR] Atualizando via run_refresh() às {datetime.now()}")
    ok, msg = run_refresh()
...
def set_schedule():
    ...
    sch_type  = request.form.get("schedule_type", "off")
    sch_hours = request.form.get("schedule_hours", "").strip()
    sch_time  = request.form.get("schedule_time", "").strip()
    sch_spec  = request.form.get("schedule_specific_times", "").strip()
    sch_pdf   = request.form.get("schedule_pdf_folder", "").strip()
    dias_str  = request.form.getlist("schedule_weekdays")

    try:
        sch_hours = int(sch_hours) if sch_hours else 24
    except ValueError:
        sch_hours = 24

    sch["type"]           = sch_type or "off"
    sch["hours"]          = sch_hours
    sch["time"]           = sch_time or "08:00"
    sch["specific_times"] = sch_spec
    sch["pdf_folder"]     = sch_pdf

    try:
        weekdays = [int(d) for d in dias_str] if dias_str else [1,2,3,4,5]
    except ValueError:
        weekdays = [1,2,3,4,5]
    sch["weekdays"] = weekdays
...
def run_refresh() -> Tuple[bool,str]:
    ...
    # lê os arquivos conforme está configurado na tela
    df_raw, source, warns0 = load_excel_df(cfg)
    if df_raw is None:
        # mensagem de erro vinda do load_excel_df (ex: Nenhum arquivo encontrado...)
        return False, source or "Falha ao ler arquivos."

    # normaliza dados para os pivôs
    ...
    # salva cache para painel/relatórios/PDF
    save_cache(df_norm, source, colmap, warns0 + warns)

    # atualiza last_refresh na config (igual usa na tela)
    cfg["last_refresh"] = datetime.now().strftime("%d/%m/%Y %H:%M")
    save_config(cfg)

    msg = f"Atualizado com {len(df_norm):,} linhas."
    print("[REFRESH]", msg)
    return True, msg
